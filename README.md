...
`{dmhct}` exists only on Github and is currently a private repository.
This means that you will need to verify your identity to install it,
which is most easily done by setting up a Github PAT for your local
machine. See
[`usethis::gh_token_help()`](https://usethis.r-lib.org/reference/github-token.html)
for more information.

Once your token is set up, you can install `{dmhct}` with:
...
the installation instructions at the beginning of this document. Note
that you will need to set up a Github account and Github PAT to do so,
since this repository is currently private.
