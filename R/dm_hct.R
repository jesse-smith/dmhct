...
dm_hct <- function(
  ...
  if (!.quiet) {
    rlang::inform(style_bold("Extracting database (1/3)"))
  }
  dm <- dm_extract(
    dm_remote = dm_remote,
    ...,
  ...
  # Standardize columns
  if (!.quiet) rlang::inform(style_bold("Standardizing table columns (2/3)"))
  dm <- dm_standardize(dm_local = dm, quiet = .quiet)
  # Combine tables
  if (!.quiet) rlang::inform(style_bold("Combining similar tables (3/3)"))
  dm <- dm_combine(dm)
  if (!.quiet) rlang::inform("Done.")
  dm
}
